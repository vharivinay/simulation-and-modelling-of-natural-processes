# -*- coding: utf-8 -*-
"""
Created on Sat Nov 14 23:48:55 2020

Modified by: Harivinay
"""
...
def PrintProgressMessage() :
    global curTime, ppm_event
    busy_markers = ["|", "/", "-", "\\", "-"]
    #print('{0:04d}      {1}\r'.format(curTime//100,\
    i = 0
    while True and ppm_event.is_set():
        i += 1
        print('{0:04d}      {1}\r'.format(curTime//100,\
                    busy_markers[i%len(busy_markers)]), end='')

#---------------------------

ppm_thread = Thread(target=PrintProgressMessage)
ppm_thread.daemon = True
ppm_thread.start()
...
for time in range(maxIter):
    ...
    if (time%100==0):
        #plt.clf()
        image_frame = plt.imshow(sqrt(u[0]**2+u[1]**2).transpose(), cmap=cm.jet, animated=True)
        #plt.savefig("vel.{0:04d}.png".format(time//100))
        list_im.append([image_frame])
#-------------  for time in range(maxIter):  -----------------------    

my_movie = animation.ArtistAnimation(fig, list_im, interval=50, blit=True,
                                repeat_delay=1000)
my_movie.save('my_movie.mp4')
plt.show()
